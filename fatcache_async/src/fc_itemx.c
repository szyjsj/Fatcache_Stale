...
itemx_removex(uint32_t hash, uint8_t *md)
{
    ...
    /****szy: here reduce the valid data ratio of the disk slab*****/
    tmp_sid = itx->sid;
    tmp_sinfo = &stable[tmp_sid];
    if(tmp_sinfo->mem == 0)
    {
		tmp_sinfo->valid_number--;
		tmp_sinfo->valid_ratio = (float)(tmp_sinfo->valid_number) / ((&ctable[tmp_sinfo->cid])->nitem);
    }
    bucket = itemx_bucket(hash);
    nitx--;
