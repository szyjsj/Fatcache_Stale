...
slab_evict(void)
{
    ...
    ****************************************************************/
	...
	if (Current_FR == LOW_FR)
	{
		...
		gc_all = (rand() / (float)(RAND_MAX)* 0.3) * c->nitem;
		for (gc_index = 0; gc_index < gc_all; gc_index++)
		{
			usleep(10);
		}
	}
...
slab_swap_addr(struct slabinfo *msinfo, struct slabinfo *dsinfo)
{
    ...
    ****************************************************************/
    ...
    dsinfo->erase_count = 0;
    dsinfo->access_frq = -1;
    dsinfo->valid_number = 0;
...
slab_drain(void)
{
   ...
   if(!GC_STATUS)
   {
	...
	if ((nfree_dsinfoq <= lowWatermark) && (Current_FR == LOW_FR))
	{
		Current_FR = HIGH_FR;
	}
